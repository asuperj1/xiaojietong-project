-- ============================================================
-- C46 · 通知表结构扩展（原 `B19` · 二阶段 W1 关键路径）
--
-- ★ 唯一真源：`campus_notice` 的扩展列（`deadline`/`materials`/`importance`）以本脚本为准；
--   旧环境缺列时用下方守卫补列，**不得**反向改定义。规范：`docs/db-migration-convention.md`。
--   字段契约与 `B20`/`C33`/`C34` 的 DAO/Pydantic 一一对应，改动需同步。
--
