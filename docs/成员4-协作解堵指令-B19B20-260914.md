...
| 谁 | 拿什么 | 为什么这么分 |
|---|---|---|
| **成员4** | `15_home_banner.sql` · `16_search_history.sql` · B20 的 **Pydantic 契约** · **3 个 `.ps1` 补 BOM** | 全是**新建、无历史数据**、**不需要碰 C++ 编译链**；且**原始方案文档里 DDL 脚本本来就是成员4 的活**（见 §3 依据） |
...
### 任务 ③：B20 的 **Pydantic 契约**（`backend/app/schemas/`）

- **交付物**：`backend/app/schemas/notice_extend.py`（若目录下已有相近文件就并入，不要新建重复模块）
- **现状（实测）**：`git grep deadline -- backend/app/schemas backend/app/models` → **0 命中**（契约完全缺失）
- **要做什么**：把 B20 的 3 个字段落成可空 Pydantic 模型，供接口响应序列化使用

**规格**（必须与 `db/sql/14_notice_extend.sql` 严格一致）：

| 字段 | Python 类型 | 可空 | 备注 |
|---|---|---|---|
| `deadline` | `datetime \| None` | ✅ | 抽取结果：报名/申请/提交截止 |
| `materials` | `str \| None` | ✅ | 多条以换行分隔 |
| `importance` | `int \| None` | ✅ | 1~5；**`None` = 未打分，必须与 `0` 区分** |

> ⚠️ **Pydantic v2**（本仓已统一 v2）。用 `Optional[...] = None`，不要用 v1 的 `Field(None)` 旧写法。
> ⚠️ 这 3 个字段**只加不删**：现有响应字段一个都不能动（向后兼容，`docs/api.md` v1.18 已这么承诺）。

**验收**：`E:/miniconda3/python.exe -c "from app.schemas.notice_extend import *"` 不报错；
且 `pytest tests -q` 仍 **72 passed**（不许引入回归）。
...
5. **不要改成员3 的文件**（`17_`/`18_`/`life_dao.cpp`/`schemas/knowledge*` 等），避免冲突。
...
#            backend/app/schemas/notice_extend.py、3 个 .ps1 的编码修正
...
git add backend/app/schemas/notice_extend.py
git commit -m "feat(backend): B20 补 deadline/materials/importance 可空字段契约"
...
> 💡 **建议拆 2 个 PR**：`15_/16_`（DDL）一个、`schemas + ps1`（后端小修）一个。
> 因为后者能**立刻解掉 PR #60 的阻断项**，不该等 DDL 讨论完。
...
- [ ] `backend/app/schemas/notice_extend.py` 能 import，3 字段均可空
- [ ] `pytest tests -q` 仍 **72 passed**（无回归）
- [ ] 3 个 `.ps1` 语法错误数 = **0**（用 §附 B 的一行命令查）
- [ ] `verify_b16_b18.ps1` 实跑 **EXIT=0** → 贴输出
- [ ] PR **base = `dev`**（截图确认）
...
成员4: schemas/notice_extend.py + 3 个 ps1 补 BOM ─→ PR-B ─→ 合入 dev
