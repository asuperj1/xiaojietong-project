...
>
> 📝 **本版变更（260914 修订）**：
> ① **新增分工**：`F11`/`F12`/`F13` 的**具体图像绘制 + 素材查找 + 抠图切图**从成员1 **转给成员4**（总表新增编号 **`D11a` 前端视觉素材中心**）；成员1 只出**素材清单**并做**接入**。
> ② **新增今日任务**：成员2 = **`B19`~`B22`**；成员1 = **`F11`~`F13`**；成员3 = **`C21` schema 草案**（解 `B21` 的卡）；成员4 = **`D11a`**。
> ③ 依赖图与收工清单已同步（见 §4 / §5）。
...
| **成员2**（后端） | PR #60 B16/B17/B18 代码完成 ✅ 但审查出 **1×P0 + 2×P1**，**不满足合并条件** | ① 修 `/life/notices` 私密行泄漏（**P0**）② 修两条**恒真空断言**（带**反向对照**）③ 修 `purge` 通配符护栏 | ④ 加 `admin_a` 夹具使 `pytest` 全绿 ⑤ **`B22` 采集合规+限速**（不依赖他人，可先做）⑥ **`B20` `sources` 契约扩展** ⑦ **`B21` 采集器骨架**（等成员3 的 `C21` 草案）⑧ **`B19`** 先定字段契约（⛔ 改表等 `D7`）⑨ 上传流式限读 / PDF `max_length` / B17 语料口径 |
| **成员3**（数据/AI） | C15/C16/C20 **三分支已推送但未开 PR**；CAC-25 已合入 `dev` ✅ | ① 给 **C15/C16/C20** 开 3 个 PR 并请人 Approve ② `fix/data01-backport-to-dev` 开 PR（恢复 P0 到 `dev`） | ③ **`C21` 配置 schema 草案**（成员2 的 `B21` **卡在它**；只需 schema + 示例配置）④ **CAC-27** ⑤ **C33** 帖子搜索索引 |
| **成员4**（DB/文档/运维） | **D7/D8/D9/D11/ENG-01 全部未开始**，而 **D7 是全队关键路径** 🔴 | ① **D7 的 5 个 SQL 脚本**（`14~18`，幂等）② `user_student_no` 空串 `''→NULL` 预处理（实测 **63 行**） | ③ **`D11a` 视觉素材**（**新增职责**）：按成员1 清单**查找→抠图→切图→产出图标/按钮资源**，首批交 `user-solid.svg` ④ D8 ER 图 ⑤ **ENG-01** CI（含 `T-02`） |
...
> 「具体图像绘制 + 素材查找 + 抠图切图」**全部转给成员4**（新编号 **`D11a` 前端视觉素材中心**）。
...
| **③ `F12` TabBar POC** | `custom-tab-bar/`：5 项 + **AI 居中凸起圆形**（图片用 `D11a`）+ 长按唤起语音；真机验凸起位置、全屏页隐藏 | ⛔ 凸起按钮图等 `D11a`；**骨架与逻辑可先写** |
...
#### 🟡 今日接着干：**`B19` ~ `B22`**（#60 修完后立即开工）

先把依赖说清楚 —— **四项里 `B22` / `B20` 今天不卡**，另两项都有前置：

| 编号 | 今天能做到什么 | 卡点 | 建议顺序 |
|---|---|---|---|
| **`B22`** 采集合规与限速 | `robots.txt` 检查 + **限速模块** + 日志可追溯。**完全不依赖他人** | 无 | **① 先做**（`B21` 的地基，先做不浪费） |
| **`B20`** `sources` 事件契约扩展 | SSE `sources` 事件加 `snippet`/`chunk_id`/`score`（**旧字段全保留**）+ 更新 `docs/api.md` | 无（与成员3 的 `C19` 同源，先把自己这半做完） | **②** |
| **`B21`** 配置驱动采集器 | 先做**采集器骨架**（读配置 → 抓取 → 解析 → 入库/去重）+ 复用 `B22` 的限速；配置**内容**等 `C21` | ⛔ **卡 `C21`**（成员3，原 W5）→ 你只需它的 **schema 形状**，已请成员3 今天先出**草案** | **③** |
| **`B19`** 通知表结构扩展 | 先定**字段契约**（`campus_notice` + `deadline`/`materials`/`importance`，**全部可空**）+ 写 DAO/Pydantic/接口层；**不动表** | ⛔ **卡 `D7`**（`14_notice_extend.sql` 未出）→ DDL 到位后改表 + 重编译 | **④**（DDL 一到就补完） |

⚠️ **字段名/类型必须与成员4 的 `14_notice_extend.sql` 逐字一致**（含可空性与默认值），否则改表后 DAO 直接报错。
建议：**今天就把字段清单发到群里让成员4 照拄**（也帮他少一次返工）。

> 💡 这个顺序的理由：`B22` → `B21` 是递进（限速模块被采集器复用）；`B20` 独立且快；`B19` 有一半必须等 DDL，所以放最后「先把不依赖 DDL 的那一半做完」。
...
#### 🟡 今日应做（新增职责）：**`D11a` 前端视觉素材中心** 🖼️

> **这一项是你今天新增的活**（自成员1 的 `F11`/`F12`/`F13` 拆分，已写进总表 §3.4）。
> **成员1 不再找图/画图/抠图**；他出**素材清单**，你按单**交货**，他拿去只做接入。

| 你要交付 | 内容 | 交付位置 |
|---|---|---|
| **图标（第一批，优先）** | 苹果**线性图标** SVG（`stroke-width:1.5`，统一线宽/尺寸/留白）；**必须含 `user-solid.svg`**（当前缺，`ui/` 里也没有） | `ui/assets/icons/` |
...
**工作方式**：**先等成员1 的素材清单**（尺寸/格式/命名/留白规则）—— 有单再抠，避免返工；
没有真图时先用**占位图 + 同尺寸**顶上，保证成员1 今天不阻工。

> 💡 优先序建议：**`user-solid.svg` 先交**（它是 `F11` 的硬缺口）→ 再交 TabBar 凸起按钮（`F12` 要）→ 再交首页 Banner/Logo（`F13` 要）。
...
flowchart LR
    ...
    D7 --> B25["成员2 · B25<br/>home_banner 表"]
    B25 --> F13["成员1 · F13 首页<br/>（先做静态部分）"]
    C28 --> B30["成员2 · B30/B26<br/>学号/搜索历史"]
    C28 --> F17["成员1 · F17 我的页"]
    B19 --> C18["成员3 · C17/C18<br/>通知抽取与打分"]
    LIST["成员1 · 素材清单<br/>尺寸/格式/命名"] --> D11A["成员4 · D11a 视觉素材<br/>查找 → 抠图 → 切图"]
    D11A --> F11["成员1 · F11 图标集成"]
    D11A --> F12["成员1 · F12 TabBar"]
    D11A --> F13
    C21["成员3 · C21 schema 草案<br/>（今日新增）"] --> B21["成员2 · B21 采集器"]
    B22["成员2 · B22 限速/robots"] --> B21
...
4. 🟠 **`C21` 草案（成员3）** —— 成员2 的 `B21` 采集器在等它的 **schema 形状**（今日新增依赖，已排进 §2.3）。
...
- [ ] **`B22`** 限速/`robots` 有可运行模块
- [ ] **`B20`** 契约扩展已改 `docs/api.md`（旧字段全保留）
- [ ] **`B19`** 字段清单**已发群**（供成员4 的 `14_` 照拄）

**成员3**
- [ ] 4 个 PR 已开（3 个 C 系列 + data01-backport），并已请人 Approve
- [ ] 至少 1 个 PR 已合入 `dev`
- [ ] **`C21` schema 草案 + 示例配置已交成员2**（解 `B21` 的卡）
...
- [ ] **`D11a` 首批素材已交**（至少 `user-solid.svg`，`F11` 在等它）
- [ ] `D8` ER 图有初稿
