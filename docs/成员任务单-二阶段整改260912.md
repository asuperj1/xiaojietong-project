# 校捷通 · 二阶段任务总表（**v5.2 · 2026-09-16 实测刷新**）

> 🔄 **v5.2 增量（2026-09-16 17:45）—— 本版新增两节**：
> ① **§0·2.2.1「成员1 立即执行：恢复 + 再部署 + 后续文件安置」** —— 前端回滚的**可直接执行的操作步骤**
>    （已实测：回退后无人再动 `miniprogram/`，仓库里有 tag `archive/dev-pre-rollback-20260916`，**一条 `git checkout` 就能整体恢复**）；
> ② **§0·2.6「开放 PR 审查意见」** —— 对当前 **8 个开放 PR** 逐个给出结论，含 **2 个阻断级 P1**：
>    `#107`（会把任务总表**回退到 v3**，且与 `#108` 冲突）与 `#104`（CI 建库清单**缺 15~19/99e**，G1 会红）。
>
> 🔄 **v5.1 增量刷新（2026-09-16 17:35）**：`dev` 前移至 **`92d9800`** ——
> **PR #105（`B32` 语音转文字）已合入**（新增 `routers/voice.py` ＋ `services/asr/` 5 文件）
> → 成员2 已进 `dev` 由 7 项更新为 **8 项**；开放 PR 更新为 **8 个**（含本 PR **#108**）。
>
...
| 项 | 实测值 |
|---|---|
| `github/dev` | **`92d9800`**（2026-09-16 17:30；含 **#105 `B32` 语音**） |
| `github/main` | `ef57504`（2026-09-13 21:12，**落后 `dev` 114 个提交**；按协议只接收来自 `dev` 的合并） |
| 本分支 | `fix/task-sheet-v5`（**已对齐 `github/dev@92d9800`**） |
| **开放 PR** | **8 个**：`#94` `#98` `#100` `#101` `#104` `#106` `#107` **`#108`（本 PR）** |
...
| **成员2**（后端/工程化） | **8**：`B16` `B17`（管线）`B18` `B20` `B24` `B25` `B26` **`B32`** | 1：`B34`(#104) | `B21` `B22` `B23` `B27` `B28` `B29` `B30` `B31` `B33` | 主干终于有后端了；但 `B21`/`B22` 的 PR **已关未合** |
...
| `routers/voice.py` ＋ `services/asr/`（5 文件：`base`/`whisper_local`/`http_remote`/`probe`）（`B32`，PR **#105**） | ✅ |
...
| 接入产物（13 ＝ 12 个 svg ＋ `README.md`） | `miniprogram/static/icons/*.svg`（`user.svg` / `user-solid.svg` / `bell.svg` / `map.svg` / `briefcase.svg` …） | **`F11`** 的**接入产物** |
...
#### 0·2.2.1 👉 成员1 立即执行：一条命令恢复 + 再部署 + 后续文件安置

> ✅ **好消息**：仓库里**已经打好了回退前的 tag**：`archive/dev-pre-rollback-20260916` = `602ef99`（= `1acbf38^`），
> 而且**回退后没有任何提交再动过 `miniprogram/`**（实测 `git log 1acbf38..github/dev -- miniprogram` 为空）
> ⇒ **不需要手工重写任何文件，`git checkout` 即可整体恢复**。

**第 1 步 · 拉分支**（`dev` 受保护，不能直推）
...
# ② 只挑这 2 个工具文件恢复：
#    ⚠️ 不要整目录 checkout tools/！dev 上有回退后新增的 verify_ddl_*.py，整目录会覆盖它们
...
| # | 检查 | 命令 | 期望 |
|---|---|---|---|
| 1 | 恢复文件数 | `git status --short` | **共 24 项** = `miniprogram/` **22** ＋ `tools/` **2**；其中 **`M` 6**、**新增 `A` 18** |
| 2 | 没有多余改动 | `git status --short` | **只出现** `miniprogram/` 与 `tools/verify_*` 两个文件；不得出现 `backend/` / `db/` / `ai/` |
| 3 | `F10` 三件套 | `Test-Path miniprogram/utils/glass.js, miniprogram/styles/tokens.wxss, miniprogram/styles/glass.wxss` | 全 `True` |
...
| 6 | 守卫工具回归 | `node tools/verify_frontend_base_url_guard.js` | **26/26**，退出码 **0** |
| 7 | 图标工具回归 | `node tools/verify_f11_icon_set.js` | 退出码 **0** |
| 8 | 玻璃探针回归 | `node tools/verify_glass_probe.js` | **12/12**，退出码 **0** |

> ⚠️ **反向对照不能省**：只跑一次「恢复后通过」并不证明工具有判别力。
> **应在恢复前先在 `dev` 上跑一次**（预期 `verify_frontend_base_url_guard.js` 直接 `MODULE_NOT_FOUND` / 或大量 `[FAIL]` → 退出码 **1**）；
> 两边都跑过才算数（这是本仓已经被踩过两次的教训：**「验证工具自己也要有反向对照」**）。
...
```
> ⚠️ **建 PR 用三段式链接**（`?expand=1&base=dev` 那种会被重定向回 `main`）：
> `https://github.com/asuperj1/xiaojietong-project/compare/dev...fix/restore-frontend-foundation?expand=1`

**第 5 步 · 后续任务的「文件安置」约定**（恢复后按这个放，别再各页各写一套）

| 任务 | 文件放哪 | 约定 |
|---|---|---|
| **`F10`** | `miniprogram/styles/tokens.wxss` · `styles/glass.wxss` · `utils/glass.js` | 已随上面恢复。**不要再新建 `theme.js`/`colors.js` 之类**，一律走 `tokens.wxss` 变量 |
| **`F11`** | 素材源 `ui/icons/**`（**只读**，属成员4/`D8`）→ 接入产物落 `miniprogram/static/icons/*.svg` | 接入时**只复制文件**，不要反过去改 `ui/icons/` 的源文件 |
| **`F12`** | `miniprogram/custom-tab-bar/index.{js,json,wxml,wxss}` ＋ `app.json` 里 `tabBar.custom = true` | 目录名**必须是 `custom-tab-bar`**（微信固定约定），且**与 `pages/` 同级** |
| **`F13`~`F21`** | 页面本体放 `miniprogram/pages/<page>/`；**共用样式只加进 `styles/`** | 页面私有样式写各自 `.wxss`，不要改 `tokens.wxss` |

> 🔴 **`F11` 的 `user-solid.svg` 有个坑（本轮实测）**：`ui/icons/navigation/` 下**有 `user.svg` 但没有 `user-solid.svg`**；
> 而 `archive` 里那个 `miniprogram/static/icons/user-solid.svg` ——**名字叫 solid、内容却是描边版**
> （`fill="none" stroke="currentColor" stroke-width="1.5"`，与 `user.svg` **逐字节相同**）⇒ **它是误命名的重复文件，不能当素材用**。
> 真正的实心版**必须新画**，参照同目录**真正实心**的 `user-group-solid.svg`（`<g fill="currentColor">`、不带 `stroke`）。
...
| **#104** | `B34` CI 流水线 | `feat/b34-ci-pipeline` → `dev` | 队友 `yl-love`（2 条评论） |
| **#107** | 封测 / 审查资产 | `test/beta-assets` → `dev` | 我方 |
| **#108** | **任务总表 v5.1（本文件）** | `fix/task-sheet-v5` → `dev` | 我方 · **已对齐 `dev`（落后 0）** |

> ✅ **今日已合入**（v5 → v5.1 期间）：**#105 `B32` 语音转文字**（`dev` 由 `ade1459` → `92d9800`）。
...
1. 🔴 **`#89` 回退未恢复（前端 18 文件 / 107 行）** —— 卡住 `F10`→`F11`→`F12`→`F13` 整条前端链，**当前无人认领** →
   **操作步骤已写好在 §0·2.2.1，成员1 直接照做即可**（实测：回退后无人再动 `miniprogram/`，可一条 `git checkout` 整体恢复）；
...
### 0·2.6 🔍 开放 PR 审查意见（2026-09-16 17:45 实测）

> 结论均由**实测**得出：`git rev-list --left-right --count`（落后/领先）· `git diff --stat`（净增行）
> · `git merge-tree --write-tree`（冲突预检）。完整报告归档在 `test/beta-assets`：
> `docs/PR94-98-100-101-审查报告260916.md` · `docs/PR97-103-审查报告260916.md` · `docs/PR92-审查报告260916.md`。

| PR | 内容 | 归属 | 落后/领先 | 合并预检 | **审查结论** |
|---|---|---|---|---|---|
| **#94** | `C34` 基线对比框架 | 我方 | 3 / 3 | ✅ 无冲突 | **Approve with comments**（1×P2 ＋ 2×P3） |
| **#98** | `C32` 数据集质量校验 | 我方 | 3 / 4 | ✅ 无冲突 | **Approve with comments**（1×P2 ＋ 2×P3；**P2 优先级最高**） |
| **#100** | `C27` 通知时间抽取 | 我方 | 3 / 2 | ✅ 无冲突 | **Approve with comments**（1×P3） |
| **#101** | 登记 `DDL-01` | 我方 | 3 / 2 | ✅ 无冲突 | **Approve**（无问题） |
| **#106** | 登记 `CAC-30` | 我方 | 3 / 1 | ✅ 无冲突 | **Approve**（纯文档 3 行，含实测对比表与修复时机依赖） |
| **#107** | 封测/审查资产 11 份 | 我方 | **84 / 11** | ❌ **CONFLICT** | **Request changes**（1×P1 阻断，见下） |
| **#108** | 任务总表 v5.x（本文件） | 我方 | 0 / 3 | ✅ 无冲突 | ⬜ **待队友 Approve**（作者不能批自己的） |
| **#104** | `B34` CI 流水线 | 队友 `yl-love` | 29 / 1 | ✅ 无冲突 | **Request changes**（1×P1 阻断，见下） |

#### 🔴 `#107` P1（阻断）：它会把任务总表**回退到 v3**

实测：`git merge-tree --write-tree github/dev github/test/beta-assets`
→ `CONFLICT (content): Merge conflict in docs/成员任务单-二阶段整改260912.md`

- 本 PR 携带的任务总表是 **v3 版**（标题写「v3 · 2026-09-14 全员重排」，`B19` 旧编号），
  而 `dev` 上已是 **`C46` 版**（#102 带入）→ 合入会**把任务表倒退**，并与 **#108** 冲突。
- **修法（推荐 ①）**：
  ① 从本 PR 中**移除**该文件（`git checkout github/dev -- docs/成员任务单-二阶段整改260912.md` 后提交）
     → 本 PR 退化为 **纯新增 11 文件**，零冲突、可立即合；
  ② 若确实要带任务表更新，须改为**合并 `dev` 后只带自己的增量**，且**等 #108 先合**。
- 其余 11 个文件（9 份审查报告 ＋ `docs/成员任务单260914序1.md` ＋ `tools/verify_glass_probe.js`）**全部是纯新增 `A`**，无风险。
- ⚠️ 本 PR **落后 `dev` 84 个提交** → 合前先 `git merge github/dev`，否则“merge-base 变更”会让批准失效。

#### 🔴 `#104` P1（阻断）：CI 的建库清单**缺 15~19 与 99e**，且不覆盖前端

实测：`.github/workflows/ci.yml` 的导入循环只列了
`01~12 · 13_index_optimize · 14_notice_extend · 99_init_data · 99b/99c/99d`，**未导入**：

| 缺失脚本 | 后果 |
|---|---|
| `15_home_banner` · `16_search_history` · `17_user_student_no` · `18_takeaway_pickup` | `home_banner`/`user_search_history`/`pickup_point` 三表不存在、`user.token_version` 不存在 → `B21`/`B22`/`C22` 的用例一合入就在 CI 挂 |
| **`19_topic_fulltext`** | `topic` 无 ngram FULLTEXT 索引。而 `backend/tests/test_topic_search.py` 是 `pytest.mark.integration`，`pytest.ini` **未过滤 marker**、`ci_gate.py` 也不带 `-m` → **它现在就会跑**；该文件 docstring 明写「缺索引时会直接报错，不用 skip 掩盖」⇒ **G1 会红** |
| `99e_poi_seed` | B13 的 8 个 POI 种子缺失 |

> ⇒ 结论：**CI 环境与 `dev` 的 DDL 不一致**，「回归门禁」会从“拦回归”退化成“自己先红”。
> **修法**：把导入清单改为（或直接打成一个）**全覆盖**：`for f in $(ls db/sql/[0-9]*.sql | sort)`，
> 或至少补上 `15~19` 与 `99e`；**改完请先在 CI 上真跑一次**（或在空库上本地复现）再请人批。

- **P2**：**未覆盖前端**。`.github/workflows/ci.yml` 只有 G1(pytest)/G2/G3 三道 Python 门禁；
  `F22`（小程序端静态检查）与 `tools/verify_glass_probe.js` / `verify_frontend_base_url_guard.js`
  都还没进流水线 → 建议加一个 `miniprogram-static` job（node 跑 `verify_*.js`，退出码非 0 即红）。
- ✅ **做得好的**：设计原则写得很清楚（“**宁可红灯，不要假绿**”），
  `pytest --junitxml` 取精确计数、`skipped > 0` 在 `--strict` 下红灯、后端没起记为 SKIP 且仍红灯、
  “用例数 = 0 → 恒真空，未执行任何断言 → 红” —— 正对本单里反复出现的「假绿 / 恒真空断言」。

#### 🟡 `#94` / `#98` 的 P2 是同一条根因（建议同批处理）

`normalize_deadline` **三份实现、口径已漂移**（实测 10 组输入 **5 组不同**）→ 已登记 **`CAC-30`**（#106）。

- **为何 P2 但优先级最高**：`#98` 口径**过严**（只认 `-`）→ 标注者写 `2026/9/30` 与 `2026-09-30` 被判**不一致**
  → **Kappa 被低估**，而 `C32` 的验收标准正是 `Kappa ≥ 0.8` ⇒ **可能得出「标注质量不达标」的错误结论**；
  而 `#94` 口径**偏宽** → F1 **被高估**。**同一份数据、两个任务反向偏**。
- 两个 PR **可各自独立合入**；但**跨模块对照测试必须等两者都合入后才写得了**（否则 `import` 不到另一个模块）。

#### ✅ 可直接合并的建议顺序（兼顾“批准不失效”）

```
① #107 先修 P1（去掉任务表）→ 合   ← 纯新增，风险最低
② #101 / #106（纯文档登记）→ 合
③ #100（C27）· #94（C34）· #98（C32）→ 合（P2 口径同批处理）
④ #108（本文件）→ 合（依赖 ② 的顺序无关，但不要与 #107 同时开）
⑤ #104（CI）→ 修完建库清单再合
```
> ⚠️ 每个 PR 合入都会让 `dev` 前移 → **后面的 PR 批准会失效**。
> 正确节奏：**先对齐 `dev` → 冻结 → 请队友 Approve → 立刻合**，不要一次批一堆。
...
| **W4 收尾** | 3 天 | ⬜ `F17` 我的页 · ⬜ `F18` 地图 · ⬜ `F19` 外卖改革 · ⬜ `F21` 驿站样式 | ⬜ `B29`/`B30` 通知接入与回填 · ✅ **`B32` 语音已合（#105）** · ⬜ `B33` 迁移脚本 · 🟡 `B34` CI（#104） | 🔴 `C29` 来源结构化（PR 被关）· ⬜ `C30` 语音服务 | ⬜ 验收材料归档 | ⬜ **UI 重构未完成** |
...
### 3.2 后端 + 工程化 · 成员2（`B16 ~ B34`；**`B19` 已转出 → `C46`（成员3）**；**已进 `dev` 8** · PR 已开待合 1 · 未开始 9）
...
| **B32** | 语音转文字接口 | `POST /voice/transcribe` | 3~10 秒中文语音转写可用；**失败给明确错误码**（不静默） | W4 | ✅ **已合入 `dev`**（PR **#105**）：`routers/voice.py` ＋ `services/asr/`（`base`/`whisper_local`/`http_remote`/`probe`，**可插拔后端**）＋ `tests/test_voice_transcribe.py` 实测均在 `dev` |
...
> 🚀 **开工第一步（不做这个，下面全是空谈）**：按 **§0·2.2.1** 恢复 `#89` 回退掉的前端地基。
> 该节含：可直接粘贴的 `git checkout` 命令 · **8 项逐条核对表** · 反向对照要求 · 提交/开 PR 姿势 ·
> 以及 `F10`/`F11`/`F12`/`F13~F21` 各自的**文件安置约定**。
...
| 6 | **语音转文字依赖微信配额** | 演示时可能不可用 | ✅ **`B32` 已合入 `dev`（#105）**，内置 `asr` **可插拔后端**（本地 `whisper_local` / `http_remote`）→ 微信侧不可用时可直接切本地；**`C30`** 负责把它服务化 |
