...
> 🔒 **当前保护现状（2026-09-13 实测修正）**：GitHub 规则集 `team-branch-protect` **现同时保护 `main` 与 `dev`**，两者均要求**必须通过 PR 合并**；`main` 另限制只接收 `dev → main` 的 PR。
> ⚠️ **`dev` 已不能再直推**（2026-09-13 实测：`git push github dev` 返回 `GH013: Changes must be made through a pull request`）。**不要再按“dev 不设保护、可直接推送”的旧结论操作**。
> ⚠️ **不要用 `git push --dry-run` 探测分支保护** —— dry-run **不触发 GitHub ruleset 校验**，会输出“推送成功”的假象（本次因此误判）。要确认只能真推，或查 `Settings → Rulesets`。
## 分支一览（完整树）
