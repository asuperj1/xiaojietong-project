 ...
 *
 * ⚠️ 本边界对的**检测力边界**（独立复核实测，勿外推）：它锁住的是**阈值常量**在已采样机型点上的
 *    取值，**不是**整条规则。若在数值比较之外再加一个按**未采样属性**分流的谓词，本组场景仍会
 *    exit 0 —— 例：`return parseFloat(matched[1]) >= 9 && !/^SM-/.test(info.model)`
 *    （三星机型被静默降级）实测仍 14/0 通过。要发现这类变体需要按属性造场景，不在本脚本范围。
 *
...
// ⭐ S4b / S4c：Android 阈值**边界对** —— 把**阈值常量**夹死在 `(8.9, 9]`。
//   ⚠️ 它锁的是阈值，**不是**整条规则（检测力边界见文件头「本边界对的检测力边界」）。
...
//   边界对的两点：下界含（9 → true）+ 下界之下（8.9 → false）。
check(
