#!/usr/bin/env python
"""C46（原 `B19`）验收：`db/sql/14~18_*.sql` 的列定义必须与 C++ DAO 引用的列**严格一致**。

为什么需要这个工具
------------------
C46（原 `B19`）的验收标准是「**幂等可重跑；与 `C22~C25` / `B20` DAO 字段严格一致**」。
...
# 本工具只关心 **C46（原 `B19`）交付 / 收敛涉及**的表：其余业务表不在本 PR 范围，
...
def collect_dao_columns(ref: str) -> list[tuple[str, str, str]]:
    ...
    # 只保留 C46 相关表（其它表的 ERROR 1146 会淹没真正要看的 ERROR 1054）
    return [(t, c, src) for (t, c), src in sorted(found.items()) if t in C46_TABLES]
...
def main() -> int:
    ...
    print(f"== C46 契约检查（原 B19）：DAO ref = {ref} ==")
